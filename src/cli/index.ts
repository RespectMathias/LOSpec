...
program
  ...
  .option('--lean', 'Force Lean 4 checks')
  .option('--no-lean', 'Skip default Lean 4 checks')
  .option('--concurrency <n>', 'Max concurrent validations (defaults to env OPENSPEC_CONCURRENCY or 6)')
  .option('--no-interactive', 'Disable interactive prompts')
  .action(async (itemName?: string, options?: { all?: boolean; changes?: boolean; specs?: boolean; type?: string; strict?: boolean; json?: boolean; lean?: boolean; noInteractive?: boolean; concurrency?: string }) => {
    try {
